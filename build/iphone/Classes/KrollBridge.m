...
NSString * OmniMobileSalesApp$ModuleRequireFormat = @"(function(exports){"
...
@implementation OmniMobileSalesAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_omnimobilesalesapp);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create OmniMobileSalesApp global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "OmniMobileSalesApp" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_omnimobilesalesapp = [[OmniMobileSalesAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_omnimobilesalesapp];
	
	NSString *_omnimobilesalesappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _omnimobilesalesappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_omnimobilesalesapp valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:OmniMobileSalesApp$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.omnimobilesalesapp.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.omnimobilesalesapp.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
